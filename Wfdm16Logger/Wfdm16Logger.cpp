...
static const char* mylogger_spec[] = {
    "implementation_id", "Wfdm16Logger",
    "type_name",         "Wfdm16Logger",
...
Wfdm16Logger::Wfdm16Logger(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_InPort("wfdm16logger_in", m_in_data),
      ...
      m_debug(false)
{
    ...
    registerInPort("wfdm16logger_in", m_InPort);

    init_command_port();
    init_state_table();
    set_comp_name("Wfdm16Logger");
}

Wfdm16Logger::~Wfdm16Logger()
{
}

RTC::ReturnCode_t Wfdm16Logger::onInitialize()
{
    if (m_debug) {
        std::cerr << "Wfdm16Logger::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t Wfdm16Logger::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int Wfdm16Logger::daq_dummy()
{
    return 0;
}

int Wfdm16Logger::daq_configure()
{
...
int Wfdm16Logger::parse_params(::NVList* list)
{
    int ret = 0;
    ...
    for (int i = 0; i < length; i += 2) {
        ...
        if (sname == "isLogging") {
            ...
            if (svalue == "yes") {
                ...
                isExistParamLogging = true;
                std::cerr << "Wfdm16Logger: Save to file: true\n";
            }
            else if (svalue == "no") {
                m_isDataLogging = false;
                isExistParamLogging = true;
                std::cerr << "Wfdm16Logger: Save to file: false\n";
            }
        }
...
int Wfdm16Logger::daq_unconfigure()
{
    std::cerr << "*** Wfdm16Logger::unconfigure" << std::endl;
    if (m_isDataLogging) {
        delete fileUtils;
        if (m_debug) {
...
int Wfdm16Logger::daq_start()
{
    std::cerr << "*** Wfdm16Logger::start" << std::endl;
    ...
    if (m_isDataLogging) {
        ...
        if (ret < 0) {
            std::cerr << "### ERROR: Wfdm16Logger: open file failed\n";
            fatal_error_report(CANNOT_OPEN_FILE);
        }
        else {
            std::cerr << "*** Wfdm16Logger: file open succeed\n";
            m_filesOpened = true;
        }
    }
    return 0;
...
int Wfdm16Logger::daq_stop()
{
    std::cerr << "*** Wfdm16Logger::stop" << std::endl;

    if (m_isDataLogging && m_filesOpened) {
        if (m_debug) {
            std::cerr << "Wfdm16Logger::stop: close files \n";
        }
...
int Wfdm16Logger::daq_pause()
{
    std::cerr << "*** Wfdm16Logger::pause" << std::endl;
    return 0;
}

int Wfdm16Logger::daq_resume()
{
    std::cerr << "*** Wfdm16Logger::resume" << std::endl;
    return 0;
}

int Wfdm16Logger::reset_InPort()
{
    bool ret = true;
    while (ret) {
        ret = m_InPort.read();
    ...
    if (m_debug) {
        std::cerr << "*** Wfdm16Logger::InPort flushed\n";
    }
    return 0;
}

void Wfdm16Logger::toLower(std::basic_string<char>& s)
{
...
int Wfdm16Logger::daq_run()
{
    ...
    if (m_isDataLogging) {
        ...
        if (ret < 0) {
            std::cerr << "### Wfdm16Logger: ERROR occured at data saving\n";
    ...
    if (m_debug) {
        ...
        if (seq_num % m_update_rate == 0) {
            std::cerr << "Wfdm16Logger: loop = " << seq_num << std::endl;
            std::cerr << "\033[A\r";
        }
    }
...
extern "C"
{
    void Wfdm16LoggerInit(RTC::Manager* manager)
    {
        RTC::Properties profile(mylogger_spec);
        manager->registerFactory(profile,
                                 RTC::Create<Wfdm16Logger>,
                                 RTC::Delete<Wfdm16Logger>);
    }
};
