...
static const char* wfdm16reader_spec[] =
{
    "implementation_id", "Wfdm16Reader",
    "type_name",         "Wfdm16Reader",
    "description",       "Wfdm16Reader component",
    "version",           "1.0",
...
Wfdm16Reader::Wfdm16Reader(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_OutPort("wfdm16reader_out", m_out_data),
      m_sock(0),
      m_read_byte_size(0),
      m_epfd(-1),
      m_ev_ret(NULL),
      ...
      m_debug(false)
{
    ...
    // Set OutPort buffers
    registerOutPort("wfdm16reader_out", m_OutPort);

    init_command_port();
    init_state_table();
    set_comp_name("WFDM16READER");
}

Wfdm16Reader::~Wfdm16Reader()
{
}

RTC::ReturnCode_t Wfdm16Reader::onInitialize()
{
    if (m_debug) {
        std::cerr << "Wfdm16Reader::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t Wfdm16Reader::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int Wfdm16Reader::daq_dummy()
{
    return 0;
}

int Wfdm16Reader::daq_configure()
{
    fprintfwt(stderr, "Wfdm16Reader::configure\n");
    //std::cerr << "*** Wfdm16Reader::configure" << std::endl;
    ...
    if (m_set_registers) {
        for (unsigned int i = 0; i < m_module_list.size(); i++) {
            ...
            if (set_delay(m_module_list[i].ip_address, m_module_list[i].delay) < 0) {
...
int Wfdm16Reader::parse_params(::NVList* list)
{
    ...
    for (int i = 0; i < len; i+=2) {
        ...
        if ( sname == "srcPort" ) {
            ...
            char* offset;
            int srcPort = (int)strtol(svalue.c_str(), &offset, 10);
            ports.push_back(srcPort);
        }
        if (sname == "windowSize") {
            if (m_debug) {
            ...
            char* offset;
            int window_size = (int)strtol(svalue.c_str(), &offset, 10);
        ...
        if (sname == "delay") {
            ...
            char* offset;
            int delay = (int)strtol(svalue.c_str(), &offset, 10);
    ...
    for (unsigned int i = 0; i < m_module_list.size(); i++) {
        ...
        std::cerr << "window_size: " << m_module_list[i].window_size << std::endl;
        std::cerr << "delay:       " << m_module_list[i].delay       << std::endl;
    }

    return 0;
}

int Wfdm16Reader::daq_unconfigure()
{
    fprintfwt(stderr, "Wfdm16Reader::unconfigure\n");
    //std::cerr << "*** Wfdm16Reader::unconfigure" << std::endl;
    m_module_list.clear();

    return 0;
}

int Wfdm16Reader::daq_start()
{
    fprintfwt(stderr, "Wfdm16Reader::start\n");
    //std::cerr << "*** Wfdm16Reader::start" << std::endl;
...
int Wfdm16Reader::daq_stop()
{
    fprintfwt(stderr, "Wfdm16Reader::stop\n");
    //std::cerr << "*** Wfdm16Reader::stop" << std::endl;
...
int Wfdm16Reader::daq_pause()
{
    std::cerr << "*** Wfdm16Reader::pause" << std::endl;

    return 0;
}

int Wfdm16Reader::daq_resume()
{
    std::cerr << "*** Wfdm16Reader::resume" << std::endl;

    return 0;
}

int Wfdm16Reader::set_data(unsigned char *read_buf, unsigned int data_byte_size)
{
...
int Wfdm16Reader::write_OutPort()
{
...
int Wfdm16Reader::get_data_byte_size(unsigned char *buf, int len)
{
    int *l;
    int rv;
    l = (int *) &buf[4];
    rv = ntohl(*l);
    //if (len < 0) {
    //    fatal_error_report(USER_DEFINED_ERROR1, "DATA LENGTH IS NAGATIVE);
    //}

    return rv;
}

int Wfdm16Reader::daq_run()
{
    if (m_debug) {
        std::cerr << "*** Wfdm16Reader::run" << std::endl;
    }
...
//    }
    ...
    for (unsigned int i = 0; i < n_readable; i++) {
        ...
        // read header only
        status = mi->Sock.readAll(mi->buf, WFDM_HEADER_BYTE_SIZE);
        if (status != DAQMW::Sock::SUCCESS) {
            if (status == DAQMW::Sock::ERROR_TIMEOUT) {
...
extern "C"
{
    ...
    {
        RTC::Properties profile(wfdm16reader_spec);
        manager->registerFactory(profile,
                    RTC::Create<Wfdm16Reader>,
                    RTC::Delete<Wfdm16Reader>);
    }
};
